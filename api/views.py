...
class TodoViewSet(GenericViewSet):
    ...
    @action(
        methods=["GET"],
        detail=False,
        url_path="get-all-todos",
        url_name="get-all-todos",
    )
    def get_all_todos(self, request: Request, *args, **kwargs):
        try:
            ...
            return Response(
                {"message": "details not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    @action(
        methods=["POST"], detail=False, url_path="create-todo", url_name="create-todo"
    )
    ...
    @action(
        methods=["PATCH"], detail=True, url_path="update-todo", url_name="update-todo"
    )
    ...
    @action(
        methods=["DELETE"], detail=True, url_path="delete-todo", url_name="delete-todo"
    )
